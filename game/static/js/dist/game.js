...
class Particle extends WxGameObject{
    constructor(background,x,y,radius,vx,vy,color,speed,move_length){
        super();
        this.background = background;
...
class Player extends WxGameObject {
    constructor(background, x, y, radius, color, speed, is_me) {
        super();
        this.background = background;
        this.ctx = this.background.game_map.ctx;
        this.x = x;
    ...
    add_listening_events() {
        let outer = this;
        this.background.game_map.$canvas.on("contextmenu", function() {
            return false;
        });
    ...
    shoot_fireball(tx, ty) {
        ...
        let radius = this.background.height * 0.01;
        let angle = Math.atan2(ty - this.y, tx - this.x);
        let vx = Math.cos(angle), vy = Math.sin(angle);
        let color = "orange";
        let speed = this.background.height * 0.5;
        let move_length = this.background.height * 1;
        new FireBall(this.background, this, x, y, radius, vx, vy, color, speed, move_length, this.background.height * 0.01);
    }
    ...
    is_attacked(angle, damage) {
        for (let i = 0; i < 20 + Math.random() * 10; i ++ ) {
            ...
            new Particle(this.background, x, y, radius, vx, vy, color, speed, move_length);
        }
        this.radius -= damage;
        if (this.radius < 10) {
            this.destroy();
    ...
    update() {
        ...
        if (!this.is_me && this.spent_time > 4 && Math.random() < 1 / 300.0) {
            let player = this.background.player[Math.floor(Math.random() * this.background.player.length)];
    ...
    on_destroy() {
        for (let i = 0; i < this.background.player.length; i ++ ) {
...
class FireBall extends WxGameObject {
    constructor(background, player, x, y, radius, vx, vy, color, speed, move_length, damage) {
        super();
        this.background = background;
        this.player = player;
        this.ctx = this.background.game_map.ctx;
    ...
    update() {
        ...
        for (let i = 0; i < this.background.player.length; i ++ ) {
            let player = this.background.player[i];
